...
{                                                                             }
{    Conversions IniFiles-TntIniFiles added for Drago project                 }
{    Copyright (c) 2019 Gilles Arcas                                          }
{                                                                             }
{*****************************************************************************}
...
procedure CopyIniToTntIni(iniFile: TMemIniFile; tntIniFile: TTntMemIniFile);
procedure CopyTntIniToIni(tntIniFile: TTntMemIniFile; iniFile: TMemIniFile; prefixSection: string);
...
procedure CopyIniToTntIni(iniFile: TMemIniFile; tntIniFile: TTntMemIniFile);
var
  listSections, listKeys: TStringList;
  iSection, iKey: integer;
  section2, key: string;
begin
  listSections := TStringList.Create;
  listKeys := TStringList.Create;
  iniFile.ReadSections(listSections);
  for iSection := 0 to listSections.Count - 1 do
    begin
      section2 := listSections[iSection];
      iniFile.ReadSection(section2, listKeys);
      for iKey := 0 to listKeys.Count - 1 do
      begin
        key := listKeys[iKey];
        tntIniFile.WriteString(section2, key, iniFile.ReadString(section2, key, ''))
      end
    end;
  listSections.Free;
  listKeys.Free
...
procedure CopyTntIniToIni(tntIniFile: TTntMemIniFile; iniFile: TMemIniFile; prefixSection: string);
var
  listSections, listKeys: TTntStringList;
  iSection, iKey: integer;
  section2, key: string;
begin
  listSections := TTntStringList.Create;
  listKeys := TTntStringList.Create;
  tntIniFile.ReadSections(listSections);
  for iSection := 0 to listSections.Count - 1 do
    begin
      section2 := listSections[iSection];
      if AnsiStartsStr(prefixSection, section2) then
      begin
        tntIniFile.ReadSection(section2, listKeys);
        for iKey := 0 to listKeys.Count - 1 do
          begin
            key := listKeys[iKey];
            iniFile.WriteString(section2, key, tntIniFile.ReadString(section2, key, ''))
          end
        end
    end;
  listSections.Free;
